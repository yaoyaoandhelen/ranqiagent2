...
<body>
  <div class="shell">
    ...
    <main>
      ...
      <section class="card section" id="treemap">
        ...
        <div class="legend">
          <span><i class="swatch" style="background:#B779F2"></i>特等奖</span>
          <span><i class="swatch" style="background:#F9738D"></i>一等奖</span>
          <span><i class="swatch" style="background:#4BAFEA"></i>二等奖</span>
          <span><i class="swatch" style="background:#43C7A4"></i>三等奖</span>
          <span><i class="swatch" style="background:#7C8DA6"></i>未获奖</span>
          <span>面积=该成果名称评审总数，成果名称评审总数越多，颜色越亮</span>
        </div>
  ...
  <script>
    ...
    const treemapHues = { "特等奖": [272, 76], "一等奖": [349, 82], "二等奖": [201, 78], "三等奖": [164, 68], "未获奖": [215, 22] };
    ...
    function treemapColor(award, total, minTotal, maxTotal) {
      const [hue, saturation] = treemapHues[award] || treemapHues["未获奖"];
      const span = Math.max(maxTotal - minTotal, 1);
      const ratio = (total - minTotal) / span;
      const adjustedHue = hue + (ratio - 0.5) * 8;
      const adjustedSaturation = saturation - 6 + ratio * 10;
      const lightness = 45 + ratio * 16;
      return `hsl(${adjustedHue.toFixed(1)} ${adjustedSaturation.toFixed(1)}% ${lightness.toFixed(1)}%)`;
    }
    ...
    function renderTreemap() {
      ...
      const body = rects.map((item, index) => {
        ...
        const fill = treemapColor(item.bestAward, item.total, minTotal, maxTotal);
