...
<body>
  ...
  <script>
    ...
    function rowsByGroup(kind, name) {
      const field = { unitBars: "第一完成单位", formBars: "成果形式", fieldBars: "所属领域" }[kind];
      return reviewedWorkRows(work => field && work[field] === name);
    }

    function tableHtml(headers, rows) {
      const body = rows.length ? rows.map(row =>
        `<tr>${row.map(cell => `<td>${esc(cell)}</td>`).join("")}</tr>`
      ).join("") : `<tr><td colspan="${headers.length}">暂无明细</td></tr>`;
      return `<div class="detail-table"><table><thead><tr>${headers.map(h => `<th>${esc(h)}</th>`).join("")}</tr></thead><tbody>${body}</tbody></table></div>`;
    }
    ...
    function showAwardDetail(award) {
      ...
      const rows = reviewedWorkRows(work => work["奖项"] === award);
      openDetail(`${award}明细`, [
    ...
    function showRankModuleDetail(kind) {
      ...
      openDetail(`${title}总览`, [
        ...
        ["统计字段", fieldName],
        ["排序规则", "按获奖数"],
      ], "", [fieldName, "申报数", "获奖数", "获奖率", "一/二/三等奖"], detailRows);
    }

    function showGroupDetail(kind, index) {
      const config = {
        unitBars: ["单位获奖数量排名", data.unitStats],
        formBars: ["成果形式获奖排名", data.formStats],
        fieldBars: ["所属领域获奖排名", data.fieldStats],
      }[kind];
      if (!config) return;
      ...
      openDetail(`${title}：${row.name}`, [
        ...
        ["获奖率", fmtPct(row.rate)],
      ], "", ["编号", "成果名称", "奖项", "第一完成单位", "成果形式", "所属领域", "综合最终得分"], rowsByGroup(kind, row.name));
    }
