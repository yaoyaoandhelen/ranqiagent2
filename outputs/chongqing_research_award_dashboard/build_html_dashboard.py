...
<body>
  <div class="shell">
    ...
    <main>
      ...
      <section class="card section" id="treemap">
        ...
        <div class="legend">
          <span><i class="swatch" style="background:#7CC7E8"></i>评审分数低</span>
          <span><i class="swatch" style="background:#A58BE6"></i>评审分数中</span>
          <span><i class="swatch" style="background:#F29A8E"></i>评审分数高</span>
          <span>面积=该成果名称评审总数；颜色=最高分</span>
        </div>
  ...
  <script>
    ...
    function treemapColor(score, minScore, maxScore) {
    ...
    function renderTreemap() {
      ...
      const scores = items.map(item => item.maxScore || item.avgScore || 0).filter(score => score > 0);
      ...
      const body = rects.map((item, index) => {
        ...
        const fill = treemapColor(item.maxScore || item.avgScore || minScore, minScore, maxScore);
